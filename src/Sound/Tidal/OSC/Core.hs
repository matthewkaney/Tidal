...
socketSendTo :: Socket -> Packet -> SockAddr -> IO ()
socketSendTo socket packet = sendAllTo socket (encode packet)
...
socketReceive' socket f = forever (recvFrom socket 8129 >>= handleOSC)
  where
    ...
    respondTo sender = mapM_ (\packet -> socketSendTo socket packet sender)
